...
def kerton_inv(kernel2, max_iter=12):
    ...
    Frob = LA.matrix_norm(kernel2, ord='fro')  # .cpu().detach().numpy())
    ...
    metric, init0 = metricFrob(I, X, A)
    # print("A0: " + str(metric.item()) + " init0: " + str(init0))
    for i in range(20):
        X = X @ (3 * I - 3 * A @ X + (A @ X) ** 2)
        # metric, FrobX = metricFrob(I, X, A)
        # print("A" + str(i + 1) + " : " + str(metric.item()) + " FrobX: " + str(FrobX))
    return X
...
def newton_inv(mat, iter=6):
    ...
    I = torch.from_numpy(np.broadcast_to(I[None, None, ...].cpu().detach().numpy(), shape=P.shape)).to(device=P.device)
    alpha = 2 / (torch.max(torch.sum(mat, dim=-1)) ** 2)
    # alpha = 0.9 * 2 / torch.max(torch.sum(mat, dim=-1), -1)[0] ** 2
    # P = P + 0.01 * I
    V = alpha * P   # [..., None, None]
    V2 = torch.from_numpy(np.broadcast_to(alpha[None, None, ...].cpu().detach().numpy(), shape=P.shape)).to(device=mat.device) * P

    beta = 0.2
    pnorm = torch.max(torch.sum(torch.abs(I - V), dim=-2))
    err_cnt = 0
    while pnorm > 0.9999 and err_cnt < 600:
        alpha *= beta
        V = torch.from_numpy(np.broadcast_to(alpha[None, None, ...].cpu().detach().numpy(), shape=P.shape)).to(device=mat.device) * P  # alpha * P
        pnorm = torch.max(torch.sum(torch.abs(I - V), dim=-2))
        err_cnt += 1
